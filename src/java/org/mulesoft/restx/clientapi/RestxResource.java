 ...
 * be used by clients to find out about component capabilities and also as a starting
 * point to create new resources, by utilizing the get_resource_template() function.
 */
public class RestxResource
{
     ...
     * @param rdesc The dictionary describing the resource. This is the dictionary
     *            returned by the server when a resource URI is accessed.
     * @throws RestxClientException
     */
    public RestxResource(RestxServer server, Map<String, ?> rdesc) throws RestxClientException
    {
        this.server = server;
        try
        {
    ...
    {
        final RestxAccessibleService service = services.get(name);
        if (service == null)
        {
    ...
    /**
     * Delete this resource from the server. This local object becomes unusable after
     * this operation.
     * 
     * @throws RestxClientException
     */
