...
/**
 * Represents information about a service of a RESTx component or resource, ready to
 * be accessed. This representation can be used by clients to find out about service
 * capabilities and also to access the service methods of the resource.
 */
public class RestxAccessibleService extends RestxService
{
    ...
    /**
     * This takes a service definition hash map as they would be received straight
     * from the server. No prior sanity checking needs to be performed on it, since
     * this constructor properly parses and interprets this map.
     * 
     ...
     */
    public RestxAccessibleService(RestxResource resource, String name, Map<String, ?> sdef)
        throws RestxClientException
    {
        super(name, sdef);
    ...
    /**
     * Store a parameter value for the subsequent service invocation. The string
     * representation of the 'value' object is stored. This parameter value will be
     * passed on the URL command line.
     * 
    ...
    /**
     * Specify an input buffer that's to be sent in the request to the service
     * method.
     * 
     ...
     * 
     * @param method The HTTP request method. If an input was set then this defaults
     *            to POST, otherwise GET.
     * @return A {@link HttpResult} object with information about the server response.
     * @throws RestxClientException
     */
     ...
     * Sends the service request to the server.
     * 
     * @param method The HTTP request method. If this is null and an input was set
     *            then this defaults to POST. If the caller requests anything but PUT
     *            or POST with a set input then an exception is thrown. If no input
     *            was specified and this is set to null then the method defaults to
     *            GET.
     * @return A {@link HttpResult} object with information about the server response.
     * @throws RestxClientException
     */
    public HttpResult access(HttpMethod method) throws RestxClientException
    {
        ...
        final StringBuilder reqUri = new StringBuilder(resource.getUri()).append("/").append(getName());

        // Assemble the query portion of the URL from the parameters
        boolean firstParam = true;
        for (final String pname : paramsVals.keySet())
        {
            if (!firstParam)
            {
                reqUri.append("&");
        ...
        final String uri = reqUri.toString();
