...
/**
 * Used by the client to encapsulate all communication with the server. When created,
 * some meta information from the server is read and stored. This object also serves
 * as a cache for often accessed information.
 */
public class RestxServer
{
    ...
    protected static final String[] REQUIRED_KEYS = {CODE_URI_KEY, DOC_URI_KEY, NAME_KEY, RESOURCE_URI_KEY,
        STATIC_URI_KEY, VERSION_KEY, SPECIALIZED_URI_KEY};
    ...
    protected Map<String, DescUriHolder> resources;
    protected Map<String, DescUriHolder> components;
    protected Map<String, DescUriHolder> specializedComponents;
        ...
        catch (final IOException e)
        {
            if (conn != null)
            {
                ...
                if (status != null)
                {
                    if (status != code)
                    {
                        throw new RestxClientException("Status code " + status
                                                       + " was expected for request to '" + fullUrl
                                                       + "'. Instead we received " + code);
                    }
                }
    ...
    /**
     * Serialize a complex object to JSON. We should be able to just use
     * {@link JSONObject} for all kinds of objects. However, it does not handle
     * strings, numbers or booleans, only maps or arrays. Therefore, we deal with
     * those manually.
     * 
    ...
    /**
     * Transcribes a JSONObject into a HashMap. Just a small helper method.
     * 
    ...
    /**
     * Transcribes a JSONArray object into an ArrayList object. Just a small helper
     * method.
     * 
     ...
     * @throws JSONException
     */
    protected static List<Object> jsonListTranscribe(JSONArray arr) throws JSONException
    {
    ...
    /**
     * Send and receive complex, JSON serialized objects. This is a wrapper around
     * {@link send}, which serializes complex objects to JSON strings and assume that
     * the response can equally be de-serialized from JSON. We only decode the
     * response as JSON if we did not get an error back.
     * 
     * @param url The URL on the server to which the request should be sent. Since
     *            this relies on an established server connection, the URL here is
     *            just the path component of the URL (including possible query
     ...
     *            method will default to POST. If a method is specified as well then
     *            it must be POST or PUT. If no data should be sent then this should
     *            be 'null'. This is assumed to be a complex object, which will be
     *            JSON serialized before sending.
     * @param method The desired HTTP method of the request.
     * @param status The expected status. If the HTTP status from the server is
     *            different than the expected status, an exception will be thrown. If
     *            no status check should be performed then set this to 'null'.
                               ...
                               HashMap<String, String> headers) throws RestxClientException
    {
        ...
        final HttpResult res = send(url, dataStr, method, status, headers);
        if (res.status < 300)
        {
    ...
    /**
     * Returns description and URI for each resource or component. An overview of
     * resources and components can be had via the same sort of structure: A
     * dictionary that maps their names to a brief summary, consisting of description
     * and URI for each resource or component.
     * 
     * @param uri The URI from which to retrieve this information.
     * @return A map consisting of DescUriHolder objects.
     * @throws RestxClientException
     */
    protected Map<String, DescUriHolder> getDescUriMap(String uri) throws RestxClientException
    {
     ...
     * @throws RestxClientException
     */
    public static void checkKeyset(Map<?, ?> hm, String[] requiredKeys) throws RestxClientException
    {
        for (final String name : requiredKeys)
        {
    ...
    /**
     * Sends the request to create a new resource on the server. Clients don't use
     * this method, but instead create a resource through a
     * {@link RestxResourceTemplate} object.
     * 
     * @param uri The full URI (starting with a '/') of the component.
     * @param rdict The dictionary with all required parameters for the resource
     *            creation.
     * @return An HttpResult object, containing the response from the server.
     * @throws RestxClientException
     */
    @SuppressWarnings("unchecked")
    protected Map<String, Object> createResource(String uri, Object rdict) throws RestxClientException
    {
        final HttpResult res = jsonSend(uri, rdict, null, 201, null);
        return (Map<String, Object>) res.data;
    }

    /**
     * Return the URI of the server to which we are connected.
     * 
     * @return The full server URI, as it was specified when the RestxServer object
     *         was created.
     */
    ...
    /**
     * Returns the name of the server. The 'name' is a token that was returned by the
     * server when we connected to it for the first time. Each server can be
     * configured with its own name. However, the URI of the server should actually
     * be sufficient to differentiate amongst them.
     * 
    ...
    /**
     * Return the doc page for the server. If we don't have a copy of it already then
     * we issue a request to the server to retrieve the doc information.
     * 
    ...
    public String getServerDoc() throws RestxClientException
    {
        if (doc == null)
        {
            final HttpResult res = jsonSend(docUri, null, null, 200, null);
            doc = (String) res.data;
        }
        return doc;
    }
    ...
    /**
     * Return high level information about all resources currently known on the
     * server. This retrieves a map from the server, which contains description and
     * URI for each resource.
     * 
     * @return Map with information about each resource.
     * @throws RestxClientException
     */
    public Map<String, DescUriHolder> getAllResourceNamesPlus() throws RestxClientException
    {
    ...
    public String[] getAllResourceNames() throws RestxClientException
    {
        ...
        final Object[] ks = resources.keySet().toArray();
        final String[] strArray = new String[ks.length];
        System.arraycopy(ks, 0, strArray, 0, ks.length);
        return strArray;
    }

    /**
     * Return high level information about all non-specialized components currently
     * known on the server. This retrieves a map from the server, which contains
     * description and URI for each component.
     * 
     * @return Map with information about each component.
     * @throws RestxClientException
     */
    public Map<String, DescUriHolder> getAllComponentNamesPlus() throws RestxClientException
    {
        return getAllComponentNamesPlus(false);
    }

    /**
     * Return high level information about all components currently known on the
     * server. This retrieves a map from the server, which contains description and
     * URI for each component.
     * 
     * @param specialized Flag indicating whether we want to see the specialized
     *            components.
     * @return Map with information about each component.
     * @throws RestxClientException
     */
    public Map<String, DescUriHolder> getAllComponentNamesPlus(boolean specialized)
        throws RestxClientException
    {
        if (specialized)
        {
    ...
    /**
     * Return the names of all non-specialized components currently known on the
     * server.
     * 
    ...
    public String[] getAllComponentNames(boolean specialized) throws RestxClientException
    {
        ...
        // Jump through hoops to get the hash-map keys as a String[],
        // because we can't just do a simple cast on the array.
        final Object[] ks = components.keySet().toArray();
        final String[] strArray = new String[ks.length];
        System.arraycopy(ks, 0, strArray, 0, ks.length);
        return strArray;
    }
    ...
    /**
     * Return an initialized {@link RestxComponent} object for the specified
     * non-specialized component.
     * 
     ...
     * 
     * @param name Name of the component.
     * @param specialized Flag indicating whether we want to get a specialized
     *            components.
     * @return Initialized {@link RestxComponent} object.
     * @throws RestxClientException
     */
    ...
    public RestxComponent getComponent(String name, boolean specialized) throws RestxClientException
    {
        ...
        final HttpResult res = jsonSend(uriPrefix + "/" + name, null, null, 200, null);
        // The data of the HTTP result should be a dictionary with the component info
        return new RestxComponent(this, (Map<String, ?>) res.data);
    }
    ...
    {
        final HttpResult res = jsonSend(resourceUri + "/" + name, null, null, 200, null);
        // The data of the HTTP result should be a dictionary with the component info
        return new RestxResource(this, (Map<String, ?>) res.data);
    }
}
